...
class RandomMachines extends React.Component {
  ...
  pickRandomBox() {
    ...
    if (!this.state.os[0]) {
      toDeleteOS.add('Linux');
    }
    if (!this.state.os[1]) {
      toDeleteOS.add('Windows');
    }
    if (!this.state.os[2]) {
      toDeleteOS.add('Other');
    }
    ...
    if (!this.state.difficulty[0]) {
      toDeleteDifficulty.add('Very Easy');
    }
    if (!this.state.difficulty[1]) {
      toDeleteDifficulty.add('Easy');
    }
    if (!this.state.difficulty[2]) {
      toDeleteDifficulty.add('Medium');
    }
    if (!this.state.difficulty[3]) {
      toDeleteDifficulty.add('Hard');
    }
    if (!this.state.difficulty[4]) {
      toDeleteDifficulty.add('Insane');
    }
      ...
      <React.Fragment>
        ...
        <Container maxWidth="md">
          ...
          <Box display="flex" justifyContent="center" ml={1} mr={1} p={1}>
            <Box m={1} p={1}>
              <FormGroup row>
                {['OSCP', 'OSCP Advanced', 'Retired', 'Recommended'].map(
                    (title, index) => (
                      <FormControlLabel
                        key={index}
                        control={
                          <Switch
                            checked={this.state.selections[index]}
          ...
          <Box display="flex" justifyContent="center">
            ...
            <Box m={1} p={1}>
              ...
              >
                ...
                <FormGroup>
                  {['Very Easy', 'Easy', 'Medium', 'Hard', 'Insane'].map(
                      (difficulty, index) => (
                        <FormControlLabel
                          key={index}
                          control={
                            <Checkbox
                              checked={this.state.difficulty[index]}
          ...
          <TargetMachine
            ...
            machineAvatars={this.props.machineAvatars}
            machineTypesIcons={this.props.machineTypesIcons}
          />
        </Container>
      </React.Fragment>
    );
